...
export class TrackPlayerModule extends PlaylistPlayer {
  ...
  constructor() {
    ...
    this.player.addEventListener('loading', () => {
      this.emitter.emit(Event.PlaybackState, { state: State.Loading });
    });
    this.player.addEventListener('loaded', () => {
      this.emitter.emit(Event.PlaybackState, { state: State.Ready });
    });
    this.player.addEventListener('buffering', ({ buffering }: any) => {
      if (buffering === true) {
        this.emitter.emit(Event.PlaybackState, { state: State.Buffering });
      }
    });
  }
    ...
    if (options.progressUpdateEventInterval) {
      ...
      this.progressUpdateEventInterval = setInterval(
        async () => {
          ...
          if (stats && lastState && lastState.state === 'playing') {
