...
import TrackPlayerModule from './TrackPlayer';
...
const warpEventResponse = (handler, event, payload) =>
  handler({ type: event, ...{ additionalKeys: payload || {} } });
  ...
  registerPlaybackService(() => {
    return async function() {
      for (let i = 0; i < events.length; i++) {
        addEventListener(
          events[i],
          warpEventResponse.bind(null, handler, events[i]),
        );
      }
    };
  });
...
export const TrackPlayer = {
  ...
  registerPlaybackService,
  registerBackendService: registerPlaybackService,
  addEventListener,
  // Player Event Types
  TrackPlayerEvents,
  // Components
