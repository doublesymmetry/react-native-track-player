...
function registerEventHandler(handler) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (eventName === 'playback-track-done') {
            emitter.addListener(eventName, warpEventResponse.bind(null, handler, 'playback-track-changed'));
        }

        emitter.addListener(eventName, warpEventResponse.bind(null, handler, eventName));
    }
