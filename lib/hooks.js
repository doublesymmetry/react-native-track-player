...
const useCurrentDownloadsProgress = (interval = 1000) => {
    const [state, setState] = useState({})

    const getDownloadProgress = async () => {
        const progressMap = await DownloadManager.getCurrentDownloadsProgress();
        setState(progressMap)
    }

    useInterval(getDownloadProgress, interval);
    return state;
}
...
module.exports.useCurrentDownloadsProgress = useCurrentDownloadsProgress;