...
const emitter =
  Platform.OS !== "android"
    ? new NativeEventEmitter(NativeModules.TrackPlayerModule)
    : DeviceEventEmitter;

const nativeDownloader = NativeModules.TrackPlayerModule;

export const addDownloadEventListener = (event, listener) => {
  return emitter.addListener(event, listener);
};

export const getDownloadDirectorySize = () => {
  return nativeDownloader.getDownloadDirectorySize();
};
class DownloadManager {
  constructor() {
    if (!DownloadManager.downloader) {
      ...
      this.setMaxSimultaneousDownloads = this.setMaxSimultaneousDownloads.bind(
        this
      );
      ...
      const downloaderEvent = new NativeEventEmitter(
        NativeModules.TrackPlayerModule
      );

      downloaderEvent.addListener(
        "onDownloadFinished",
      ...
      storageService.getAllKeyValuePairs(this.tenant).then(
        (downloads) => {
          ...
          _.forEach(downloads, (download) => {
            if (download[1] && download[1].downloadID) {
  ...
  setMaxSimultaneousDownloads(maxSimultaneousDownloads) {
    if (Platform.OS === "ios") {
      ...
      this.nativeDownloader.setMaxSimultaneousDownloads(
  ...
  createNewDownload(url, downloadID, title, assetArtworkURL, bitRate = 0) {
    let download = this.downloads.find(
  ...
  onDownloadProgress(data) {
    ...
    if (download.onDownloadProgress) {
  ...
  onDownloadFinished(data) {
    ...
    if (download.onDownloadFinished) {
  ...
  onDownloadCancelled(data) {
    ...
    _.remove(
      this.downloads,
      (download) => download.downloadID === data.downloadID
    );
      ...
      if (_.isArray(listenerObject.downloadIDs)) {
        if (_.includes(listenerObject.downloadIDs, downloadID)) {
          listenerObject.listener(
            this.getDownload(listenerObject.downloadIDs, true)
          );
        }
  ...
  removeUpdateListener(listener) {
    _.remove(
      this.updateListeners,
      (listenerObject) => listenerObject.listener === listener
    );
  }
  ...
  getDownload(downloadIDs, returnWithLabels = false) {
    ...
    if (returnWithLabels) {
      ...
      _.forEach(matchedDownloads, (matchedDownload) => {
        matchedDownloadsWithLabels[
          matchedDownload.downloadID
        ] = matchedDownload;
      });
  ...
  checkIfStillDownloaded() {
    ...
    this.nativeDownloader
      .checkIfStillDownloaded(downloadIDs)
      .then((downloadedDownloadIDs) => {
        if (!_.isEmpty(downloadedDownloadIDs)) {
          let deletedDownloadIDs = _.difference(
            downloadIDs,
            downloadedDownloadIDs
          );
          _.forEach(deletedDownloadIDs, (downloadedDownloadID) => {
            ...
            if (download && download.state !== DOWNLOAD_STATES.failed)
              download.delete();
          });
        }
      });
  ...
  getCurrentDownloadsProgress(){
      return this.nativeDownloader.getCurrentDownloadsProgress();
  }

  updateDownloadCreds(downloadID, queryParam) {
    this.nativeDownloader.updateDownloadCreds(downloadID, queryParam);
  }
